...
package Img::ColorChange {
    ...
    sub nearest_color {
        my ($src_color) = @_;
        my @candidates = @{safeist_web_colors()};
        my $min_dist = color_distance( $src_color, $candidates[0] );
        my $choose = 0;
        for my $i (1..$#candidates) {
            my $dist = color_distance($src_color, $candidates[$i]);
            $choose = $i if $dist < $min_dist;
        }
